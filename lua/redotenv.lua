-----------------------------------------------------------------------
-- 安裝 LuaRocks 模組：dotenv
-- $ luarocks install dotenv --local
-- ❯ luarocks show dotenv
--
-- dotenv 1.2.1-1 - A .env parser for Lua
--
-- A .env parser for Lua
--
-- Compatible with any Lua version, including LuaJIT! Usage of this module can be
-- found in Homepage of this module
--
-- License:        MIT
-- Homepage:       https://github.com/UrNightmaree/dotenv-lua
-- Installed in:   /Users/alanjui/.luarocks
--
-- Modules:
--         dotenv (/Users/alanjui/.luarocks/share/lua/5.4/dotenv.lua)
--
-- Depends on:
--         lua >= 5.1 (using 5.4-1)
--
...
local function check_arg(func, tbl)
	...
	for i = 1, #tbl, 2 do
		if type(tbl[i]) ~= tbl[i + 1] then
			error(("bad argument #%d to '%s' (%s expected, got %s)"):format(argc, func, tbl[i + 1], type(tbl[i])))
		end
		argc = argc + 1
	end
end
