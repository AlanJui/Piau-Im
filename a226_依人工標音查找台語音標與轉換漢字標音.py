        ...
        if tai_gi_im_piau != "":
            ...
            if tai_gi_im_piau != "":
                ...
                if tai_gi_im_piau != "":
                    ...
                    cell.offset(-2, 0).value = ""  # 清空【人工標音】欄【=】
                    print(
                        f"【{cell.value}】的【人工標音】，在【人工標音字庫】找不到，改用【標音字庫】中的【台語音標】替代。"
                    )
                    sing_kong = True
                else:
...
def jin_kang_piau_im_ca_taigi_im_piau(wb) -> int:
    ...
    try:
        ...
        # 建立字庫工作表
        jin_kang_piau_im_ji_khoo, piau_im_ji_khoo, khuat_ji_piau_ji_khoo = (
            _initialize_ji_khoo(
                wb=wb,
...
def test_jin_kang_piau_im_ca_taigi_im_piau():
    """測試 HanJiTian 類別"""
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="依【人工標音】查找【台語音標】並轉換成【漢字標音】",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用範例：
  python a226.py          # 執行一般模式
  python a226.py -test    # 執行測試模式
""",
    )
    parser.add_argument(
        "-test",
    ...
    if test_mode:
        # 執行測試
        test_jin_kang_piau_im_ca_taigi_im_piau()
    else:
        # 從 Excel 呼叫
        sys.exit(main())
