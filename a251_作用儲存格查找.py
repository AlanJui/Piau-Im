...
class CellProcessor:
    ...
    def _process_han_ji(
        ...
        #--------------------------------------------------------------------------
        # 將查詢結果，在 Console 中全部列出
        #--------------------------------------------------------------------------
        print(f"\n漢字【{han_ji}】查詢結果：")
        print("-" * 60)
        for idx, entry in enumerate(result, 1):
            # 取得台語音標（假設 result 中每個 entry 有 tai_gi_im_piau 或類似的欄位）
            siong_yong_to = ''
            if isinstance(entry, dict):
                # tai_gi_im_piau = entry.get('piau_im', entry.get('tai_gi_im_piau', ''))
                tai_gi_im_piau = entry.get('台語音標', 'N/A')
                siong_yong_to = entry.get('常用度', 'N/A')
            else:
                # 如果是其他格式，嘗試轉換
                tai_gi_im_piau = str(entry)

            print(f"  {idx}. {tai_gi_im_piau} (常用度: {siong_yong_to})")
        print("-" * 60)

        return f"【{han_ji}】找到 {len(result)} 個讀音", False
