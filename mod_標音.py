...
def separate_tone(im_piau):
    ...
    tones = "".join(
        c for c in decomposed if unicodedata.category(c) == "Mn" and c != "\u0358"
    )
...
def apply_tone(im_piau, tone):
    ...
    if "ere" in lower:
        ...
        return unicodedata.normalize(
            "NFC", im_piau[: idx + 1] + tone + im_piau[idx + 1 :]
        )
    ...
    for i in range(len(lower) - 1):
        if lower[i] == "i" and lower[i + 1] == "u":
            return unicodedata.normalize(
                "NFC", im_piau[: i + 2] + tone + im_piau[i + 2 :]
            )
    ...
    if best_idx != -1:
        return unicodedata.normalize(
            "NFC", im_piau[: best_idx + 1] + tone + im_piau[best_idx + 1 :]
        )
...
def tng_im_piau(
    im_piau: str, po_ci_tai_sia: bool = False, kan_hua: bool = False
) -> str:
    """
...
class PiauIm:
    ...
    TONE_MARKS = {
        "十五音": {1: "一", 2: "二", 3: "三", 4: "四", 5: "五", 7: "七", 8: "八"},
        "方音符號": {
            1: "",
            2: "ˋ",
            3: "˪",
    ...
    def BP_zu_im(self, siann_bu, un_bu, tiau_ho):
        ...
        # 將【閩拚音標】轉換成【閩拚注音】
        zu_im_siann, zu_im_un, zu_im_tiau = convert_bp_siann_un_tiau_to_zu_im(
            bp_siann, bp_un, bp_tiau
        )
    ...
    # 轉換【漢字標音】
    # ================================================================
    def han_ji_piau_im_tng_huan(
        self, piau_im_huat: str, siann_bu: str, un_bu: str, tiau_ho: str
    ) -> str:
        """選擇並執行對應的注音方法"""
        if piau_im_huat == "十五音":
    ...
    def hong_im_tng_tai_gi_im_piau(self, siann, un, tiau):
        ...
        # 查詢【聲母對照表】轉換【聲母】
        self.cursor.execute(
            "SELECT 台語音標 FROM 聲母對照表 WHERE 方音符號 = ?", (siann,)
        )
...
def convert_tl_to_tlpa(tai_lo: str) -> Optional[str]:
    ...
    siann_bu_pattern = re.compile(
        r"(b|c|z|g|h|j|kh|k|l|m(?!\d)|ng(?!\d)|n|ph|p|s|th|t|Ø)"
    )
...
def TL_Tng_Zu_Im(siann_bu, un_bu, siann_tiau, cursor):
    ...
    cursor.execute(
        "SELECT 方音符號調符 FROM 聲調對照表 WHERE 台羅調號 = ?", (siann_tiau,)
    )
...
# Helper Functions
# =======================================================================================
def tlpa_tng_han_ji_piau_im(
    piau_im: PiauIm, piau_im_huat: str, tai_gi_im_piau: str
) -> str:
    ...
    try:
        ...
        if not han_ji_piau_im:  # 傳回：空字串、""、False、0，表示【漢字標音】轉換失敗
            logging_exc_error(
                msg=f"【台語音標】：[{tai_gi_im_piau}]，無法以【{piau_im_huat}】漢字標音法轉換！",
                error=None,
            )
            han_ji_piau_im = ""
    except Exception as e:
