...
DEFAULT_SRT_PATH = Path.home() / "_tmp" / "_tmp.srt"
...
def resolve_srt_sheet(wb, sheet_arg: str | None):
    ...
    header = active.range((1, SRT_COL)).value
    ...
    for sheet in wb.sheets:
        value = sheet.range((1, SRT_COL)).value
...
def collect_srt_blocks(sheet) -> list[str]:
    ...
    row = START_ROW
    while True:
        cell_value = sheet.range((row, SRT_COL)).value
...
def process(wb, args) -> int:
    ...
    try:
        ...
        logging_process_step(f"來源欄：{xw.utils.col_name(SRT_COL)}（自第 {START_ROW} 列起）")
...
使用範例：
  python b200_製作字幕檔.py
      # 預設：作用中活頁簿，輸出至 {DEFAULT_SRT_PATH}
...
""",
    ...
    parser.add_argument(
        ...
        dest="output",
        default=None,
        help=f"輸出 SRT 路徑（預設：{DEFAULT_SRT_PATH}）",
    )
