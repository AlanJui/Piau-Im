"""
a340_以漢字注音工作表之人工標音更新標音字庫.py v0.0.1

在【標音字庫】工作表，更新某【漢字】之【台語音標】，本可在【校正音標】欄填入「更正後之台語音標」
，然後再執行 a330.py 程式，透過這種方式來更新。
但此種方式，在搜尋欲變更之【漢字】時，不若直接在【漢字注音】工作表瀏覽及搜索。故此程式的
功用在於：若欲變更之【漢字】，在同一篇文章中，沒有「一字多音」的特殊情況，則使用者可在瀏
覽【漢字注音】工作表時，借由前、後文關係，來判斷【自動標音作業】，對【漢字】標注之【台語音標】
是否有錯，需要加以更正。
而使用者在【漢字注音】工作表所填入的【人工標音】，會自動填入【標音字庫】工作表對映的【資料紀錄】
之中。然後，再取【資料紀錄】中【座標】欄的資料，依【座標清單】指向【漢字注音】工作表的各個
【座標】，一一更新相同漢字的【台語音標】。
"""
...
def process(wb, args) -> int:
    ...
    try:
        ...
        # -------------------------------------------------------------------------
        # 1. 在【人工標音字庫】工作表，留下紀錄：
        #       在【人工標音字庫】工作表，記錄【漢字注音】工作表的那個【漢字】為【人工標音】。
        # -------------------------------------------------------------------------
        # a. 在【人工標音字庫】工作表記錄的資料：
        #    資料來源取自【漢字注音】工作表的【作用儲存格】
        #     - 漢字= 取自【作用儲存格】的【漢字】儲存格
        #     - 台語音標= 取自【作用儲存格】的【台語音標】儲存格
        #     - 校正音標= "N/A"
        #     - 座標= 取自【作用儲存格】的【漢字座標】 (row, col) 儲存格
        # b. 判斷【人工標音字庫】工作表，是否已有紀錄的方法：
        # 依【作用儲存格】取得之【漢字】及【座標】資料，在【人工標音字庫】字典（jin_kang_piau_im_ji_khoo_dict）
        # 搜索。
        # -------------------------------------------------------------------------

        # 自 jin_kang_piau_im_ji_khoo_dict 取得【已有的資料紀錄】
        old_jin_kang_entry = (
            xls_cell.jin_kang_piau_im_ji_khoo_dict.get_entry_by_han_ji_and_coordinate(
                han_ji=han_ji, coordinate=(row, col)
            )
        )
        # 若是找到【已有的資料紀錄】，自【資料紀錄】取得原有的【台語音標】
        old_jin_kang_tai_gi_im_piau = (
            old_jin_kang_entry.get("tai_gi_im_piau") if old_jin_kang_entry else None
        )

        # -------------------------------------------------------------------------
        # 更新【人工標音字庫】工作表紀錄的作法，分兩階段：
        # -------------------------------------------------------------------------
        # 1. 先在【人工標音字庫】字典，新增一筆【資料紀錄】；
        # 2. 然後將【人工標音字庫】字典，原有的【資料紀錄】刪除。
        # -------------------------------------------------------------------------
        ...
        # 2. 然後將【人工標音字庫】字典，原有的【資料紀錄】刪除。
        # 若在 jin_kang_piau_im_ji_khoo_dict 字典，找到【資料紀錄】，則將此原有的【資料紀錄】
        # 刪除。
        ...
        # -------------------------------------------------------------------------
        # 2. 更新【標音字庫】工作表的【資料紀錄】：
        #       依【作用儲存格】取得之【漢字】、【台語音標】及【座標】資料，更新【標音字庫】
        # 工作表的資料紀錄。【座標】欄的資料，表：所有指向【漢字注音】工作表的【座標清單】。
        # -------------------------------------------------------------------------

        # 依【作用儲存格】取得之【漢字】及【座標】資料，在【標音字庫】字典，搜索是否留有【資料紀錄】。
