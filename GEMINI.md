...
- **PRG-a999**: 自動掃描 docs 目錄產生 index.html，並負責在各文章 HTML 中注入導覽列連結。
...
## 7. 程式開發與 UI/UX 規範 (2026-03-12 增訂)

- **監控輸出 (Monitoring)**: 嚴禁移除 Console 中的「正在處理漢字」進度輸出。開發者必須保留座標、漢字及標音對應（含轉換後結果）的即時顯示，以利查檢程式執行狀態及目前進度。
- **標音存儲 (Data Integrity)**: 輸出的 HTML 中，`<ruby>` 標籤必須包含 `data-tlpa` 屬性（存儲原始台語音標），以便 JavaScript 進行動態切換。
- **TLPA+ 標準**: 系統內部轉換與資料鉤子應優先以 TLPA+ (c, z) 作為聲母標準，避免多字母聲母（如 tsh）在 JavaScript 切分時產生歧義。
- **標籤規範 (Tag Usage)**:
    - **【方音符號】**: 必須顯示於 `<rtc>` 標籤內。
    - **【其餘標音方式】**: (含十五音、台羅、白話字等) 必須顯示於 `<rt>` 標籤內。
- **切換器介面 (Switcher UI)**:
    - 應提供「預設標準方案」：方音符號、台語音標、閩拼方案、十五音、十五音+方音符號、台語音標+方音符號、閩拼方案+方音符號。
    - 應提供「使用者自訂」：允許使用者分別選擇漢字上方與右方的標音方式。
- **防禦性邏輯**: 在產生檔名或索引時，必須自動過濾或清除「None」字串，確保 UI 潔淨。
