    ...
    for row_group_index, base_row in enumerate(row_starts):
        ...
        for col_index in range(4, 19):  # D(4) 到 R(18)
            try:
                ...
                typing_sheet.range(f'E{current_row}').api.Value2 = str(siann)
                typing_sheet.range(f'F{current_row}').api.Value2 = str(un)
                typing_sheet.range(f'G{current_row}').api.Value2 = str(tiau)
